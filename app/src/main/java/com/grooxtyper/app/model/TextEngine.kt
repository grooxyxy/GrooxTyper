...
class TextEngine {
    ...
    fun renderTextToBitmap(width: Int, height: Int, config: StackableTextConfig, position: Offset): Bitmap {
        ...
        for (line in lines) {
            val textBoundsWidth = paint.measureText(line)

            // Stack 1: Drop Shadow
            if (config.hasShadow) {
                val shadowPaint = Paint(paint).apply {
                    color = config.shadowColor
                    alpha = (config.shadowOpacity * (Color.alpha(config.shadowColor) / 255f) * 255).toInt().coerceIn(0, 255)
            ...
            if (config.hasOutline) {
                val outlinePaint = Paint(paint).apply {
                    ...
                    alpha = (config.outlineOpacity * (Color.alpha(config.outlineColor) / 255f) * 255).toInt().coerceIn(0, 255)
            ...
            val fillPaint = Paint(paint).apply {
                ...
                alpha = (config.opacity * (Color.alpha(config.textColor) / 255f) * 255).toInt().coerceIn(0, 255)
                ...
                if (config.hasGradient) {
                    ...
                    val x1 = position.x
                    val y1 = currentY - config.fontSize
                    val x2 = position.x + (textBoundsWidth * Math.cos(angleRad)).toFloat()
                    val y2 = (currentY - config.fontSize) + (config.fontSize * Math.sin(angleRad)).toFloat()
